function PPI = my_spm_peb_ppi(SPM, ppiflag, voi,cond, weight,name)
% ppiflag  = 'simple deconvolution',...
%            'psychophysiologic interaction',...
%    		 'physiophysiologic interaction'}
% voi      =  specify the VOI files
% cond     =  vector including the number of the
%               regressors/condition to be included
% weight   = the wieght given to each of these regressors
% name     = name of PPI file to be saved
...
switch ppiflag
    ...
    case  'psychophysiologic interaction'  % get hemodynamic response
        ...
        U.u    = [];
        U.w    = [];
        for  i = 1:length(cond) %length(SPM.Sess.U(1).name)
            % %         for  j = 1:length(Sess.U(i).name)
            % %             str   = ['include ' Sess.U(i).name{j} '?'];
            % %             if spm_input(str,3,'y/n',[1 0])
            U.u             = [U.u Sess.U(cond(i)).u(33:end,1)];
            U.name{end + 1} = Sess.U(cond(i)).name{1};
            % %             str             = 'Contrast weight';
            U.w             = [U.w weight(i)] %%[U.w spm_input(str,4,'e',[],1)];
            %end
            %end
        end
...
%-------------------------------------------------------------------------
PPI.name    = name;%%spm_input('Name of PPI',3,'s','PPI');
